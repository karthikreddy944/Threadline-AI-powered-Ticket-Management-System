export default function Textarea({ label, hint, error, id, className = "", ...props }) {
  return (
    <div className={`flex flex-col gap-1.5 ${className}`}>
      ...
      <textarea
        id={id}
        className="min-h-28 resize-y rounded-md border border-line-strong bg-surface px-3 py-2 text-[13px] text-ink placeholder:text-ink-faint outline-none transition-colors focus:border-accent focus:ring-2 focus:ring-accent-line"
        {...props}
      />
